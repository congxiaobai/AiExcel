    ...
    const body = {
        ...
        input: {
            messages: [
                {
                    role: "system",
                    content: `你是一个数据高手。以下对象是Excel表格的结构化表示，括号内为注释。{
                        页签名(页签的标识符):{
                            cellData(表示本页签内单元格的数据):{
                    ...
                    }}}表示的'sheet1'页的第一行第一列的值为'hello'。请理解这个逻辑。`
                },
                {
                    role: "user",
                    content: `请处理一段数据：${promotText}。数据的内容为：${JSON.stringify(data)}。 
                    另外当用户说A列的时候，表示的是索引为0的列，用户说B列的时候，表示的是索引为1的列。以此类推。请保持正确的行列索引，并将结果返回给我，不要任何的注释和说明，只返回最终有变化的数据部分。`
                }
            ]
        },
        parameters: {
